...
// Configuration
const EXECUTABLE_PATH = path.resolve(__dirname, '.build/release/SystemAudioDump');
const PCM_OUTPUT_PATH = path.resolve(__dirname, 'output.pcm');
const WAV_OUTPUT_PATH = path.resolve(__dirname, 'output.wav');
const VENV_DIR = path.resolve(__dirname, 'audio-go/venv');
const PYTHON_EXEC_PATH = path.resolve(VENV_DIR, 'bin/python3');
const TRANSCRIPT_SCRIPT_PATH = path.resolve(__dirname, 'audio-go/model/transcribe.py');
...
function transcribeWavFile(wavPath) {
    return new Promise((resolve, reject) => {
        ...
        const transcriptionProcess = spawn(PYTHON_EXEC_PATH, [TRANSCRIPT_SCRIPT_PATH], {
            cwd: path.resolve(__dirname, 'audio-go')
        });
